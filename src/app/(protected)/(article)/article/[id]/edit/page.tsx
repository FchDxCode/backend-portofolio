...
export default function ArticleEditPage() {
  ...
  const { updateArticle, updateArticleImage } = useArticles();
  ...
  // Fetch article data on component mount - fixed with type assertions
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        if (article) {
          // Force type as any to avoid the type errors
          const articleAny = article as any;
          
          // Create form data from article
          const formData: FormData = {
            // Use empty objects as fallbacks if properties don't exist
            title: { 
              id: articleAny.title?.id || "", 
              en: articleAny.title?.en || "" 
            },
            preview_description: { 
              id: articleAny.preview_description?.id || "", 
              en: articleAny.preview_description?.en || "" 
            },
            description: { 
              id: articleAny.description?.id || "", 
              en: articleAny.description?.en || "" 
            },
            article_category_id: articleAny.article_category_id || 0,
            // Use type assertions for tags
            article_tag_ids: articleAny.tags ? articleAny.tags.map((tag: any) => tag.id) : [],
            minute_read: articleAny.minute_read || 0,
            is_active: articleAny.is_active || false,
            post_schedule: articleAny.post_schedule 
              ? new Date(articleAny.post_schedule).toISOString().split('T')[0] 
              : new Date().toISOString().split('T')[0],
            slug: articleAny.slug || "",
            meta_title: articleAny.meta_title || "",
            meta_description: articleAny.meta_description || "",
            canonical_url: articleAny.canonical_url || "",
            og_title: articleAny.og_title || "",
            og_description: articleAny.og_description || "",
            og_image: articleAny.og_image || ""
          };
          
          setFormData(formData);
          setCurrentImage(articleAny.image || null);
        } else {
  ...
  // Handle image upload in rich editor
  const handleImageUpload = async (file: File): Promise<string> => {
    try {
      if (!file) throw new Error("File tidak valid");
      
      // Upload to temp folder since we're just inserting into the content
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update article with the edited data
      await updateArticle(articleId, articleData, article_tag_ids);
      
      // Upload new image if available
      if (newImage) {
        await updateArticleImage(articleId, newImage);
      }
              ...
              <FormSection title="Gambar Artikel">
                {currentImage && !newImage && (
                  <div className="mb-4">
                    ...
                    <div className="relative rounded-md overflow-hidden w-full h-40">
                      <img 
                        src={currentImage} 
