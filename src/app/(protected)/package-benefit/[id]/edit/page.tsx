...
export default function PackageBenefitEditPage() {
  const router = useRouter();
  const params = useParams();
  const id = parseInt(params.id as string);
  const { updateBenefit } = usePackageBenefit();
  ...
  useEffect(() => {
    const fetchBenefit = async () => {
      try {
        setIsLoading(true);
        const benefit = await PackageBenefitService.getById(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating package benefit:', err);
